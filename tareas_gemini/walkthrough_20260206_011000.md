...
- **Association Label**: Fixed a bug where the AppBar would briefly show "Asociación desconocida" on app restart. This was caused by the system attempting to use an internal permission flag (`superadmin_access`) as a real association.
...
### 8. Fix: Article Visibility Rules

Resolved issues regarding article visibility in both Edit and Read modes, ensuring correct data segregation based on roles.

- **Superadmin (Read Mode)**: Can now see ALL published articles on the home screen, regardless of association. This fixes the "missing articles" issue.
- **Superadmin (Edit Mode)**: Sees ALL articles (any status) from ALL associations and users.
- **Admin (Edit Mode)**: Sees all articles within their assigned association(s).
- **Editor (Edit Mode)**: Sees ONLY articles they created within their association, protecting other users' drafts.

## Verification Results

### Manual Test (Visibility)

- [x] **Superadmin**: Newly created "general" articles appear on Home immediately.
- [x] **Superadmin**: Articles from other associations appear on Home (Read Mode).
- [x] **Editor**: Switching to Edit Mode shows only my own articles.
- [x] **Admin**: Switching to Edit Mode shows all articles for the association.
