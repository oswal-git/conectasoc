...
class SettingsPage extends StatelessWidget {
  ...
  String? _getAssocId(BuildContext context) {
    final authState = context.read<AuthBloc>().state;
    if (authState is AuthAuthenticated) {
      return authState.currentMembership?.associationId;
    }
    return null;
  }
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<SettingsBloc>()
        ..add(LoadSettingsData(assocId: _getAssocId(context))),
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Configuración'),
          actions: [
            IconButton(
              tooltip: 'Verificar Notificaciones Ahora',
              icon: const Icon(Icons.notifications_active),
              onPressed: () async {
                final notificationService = sl<NotificationService>();
                SnackBarService.showSnackBar(
                    'Verificando notificaciones locales...');
                final result = await notificationService.checkNow();
                if (result) {
                  SnackBarService.showSnackBar('Verificación completada.');
                } else {
                  SnackBarService.showSnackBar('Error o sin tareas web.',
                      isError: true);
                }
              },
            ),
  ...
  Widget _buildCategoryTile(BuildContext context, CategoryEntity category,
    ...
    return ExpansionTile(
      title: Text(category.name + (category.isSystem ? ' (Sistema)' : '')),
          ...
          child: Column(
            children: [
              ...subcategories.map((sub) => ListTile(
                    title: Text(sub.name + (sub.isSystem ? ' (Sistema)' : '')),
                    trailing: sub.isSystem
                        ? null
                        : Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
            ...
            ElevatedButton(
              onPressed: () {
                if (formKey.currentState!.validate()) {
                  ...
                  final assocId = _getAssocId(context);
                  if (assocId == null) {
                    SnackBarService.showSnackBar(
                        'No se puede añadir sin una asociación seleccionada',
                        isError: true);
                    return;
                  }
                  if (isCategory) {
                    context
                        .read<SettingsBloc>()
                        .add(AddCategory(name, assocId));
                  } else {
                    context
                        .read<SettingsBloc>()
                        .add(AddSubcategory(name, categoryId!, assocId));
                  }
